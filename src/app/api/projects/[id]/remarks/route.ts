...
import { addProjectRemarkSchema } from "@/lib/validators/project";
import { getRemarks, addRemark } from "@/lib/services/project.service";
...
export const POST = apiHandler(async (req, { session, params }) => {
  ...
  const body = addProjectRemarkSchema.safeParse(await req.json());
  if (!body.success)
