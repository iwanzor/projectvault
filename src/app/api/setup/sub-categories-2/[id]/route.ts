...
import { updateSubCategory2Schema } from "@/lib/validators/setup";
import { getSubCategory2ById, updateSubCategory2, deleteSubCategory2 } from "@/lib/services/product-category.service";
import { ValidationError } from "@/lib/errors";

export const GET = apiHandler(async (req, { session, params }) => {
  checkSetupPermission(session, "viewAll");
  const subCat = await getSubCategory2ById(Number(params!.id));
  return NextResponse.json(subCat);
});

export const PUT = apiHandler(async (req, { session, params }) => {
  checkSetupPermission(session, "canEdit");
  const body = updateSubCategory2Schema.safeParse(await req.json());
  if (!body.success) throw new ValidationError("Validation failed", body.error.flatten().fieldErrors);
  const subCat = await updateSubCategory2(Number(params!.id), body.data);
  return NextResponse.json(subCat);
});

export const DELETE = apiHandler(async (req, { session, params }) => {
  checkSetupPermission(session, "canDelete");
  await deleteSubCategory2(Number(params!.id));
