...
export default function GinDetailPage() {
  ...
  const ginId = Number(params.id);

  const [isEditing, setIsEditing] = React.useState(false);
  const [deleteDialogOpen, setDeleteDialogOpen] = React.useState(false);

  const { data: gin, isLoading } = useQuery({
    queryKey: ["gin", ginId],
    queryFn: () => fetchApi<GIN>(`/api/warehouse/gin/${ginId}`),
    enabled: !!ginId,
  });
  ...
  const form = useForm<GINFormData>({
    ...
    defaultValues: { requestedBy: "", projectNo: "", ginDate: "", description: "", details: [] },
  });

  React.useEffect(() => {
    if (gin && isEditing) {
      form.reset({
        requestedBy: gin.requestedBy ?? "",
        projectNo: gin.projectNo ?? "",
        ginDate: gin.ginDate?.split("T")[0] ?? "",
        description: gin.description ?? "",
        details: gin.details.map((d) => ({
  ...
  const updateMutation = useMutation({
    mutationFn: (data: GINFormData) =>
      fetchApi<GIN>(`/api/warehouse/gin/${ginId}`, {
        method: "PUT",
        body: JSON.stringify({ ...data, details: data.details.map((d, i) => ({ ...d, lineNo: i + 1 })) }),
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["gin", ginId] });
      queryClient.invalidateQueries({ queryKey: ["gins"] });
      toast.success("GIN updated"); setIsEditing(false);
    },
    onError: (error: Error) => toast.error(error.message),
  });

  const deleteMutation = useMutation({
    mutationFn: () => fetchApi(`/api/warehouse/gin/${ginId}`, { method: "DELETE" }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["gins"] });
      toast.success("GIN deleted"); router.push("/warehouse/gin");
    },
    onError: (error: Error) => toast.error(error.message),
  });
  ...
  if (isLoading) return <div className="flex items-center justify-center h-64"><div className="text-zinc-500">Loading GIN...</div></div>;
  if (!gin) return <div className="flex flex-col items-center justify-center h-64 gap-4"><div className="text-zinc-500">GIN not found</div><Button variant="outline" onClick={() => router.push("/warehouse/gin")}>Back to List</Button></div>;

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" onClick={() => router.push("/warehouse/gin")}><ArrowLeft className="h-5 w-5" /></Button>
          <div>
            <h1 className="text-2xl font-semibold">{gin.ginNo}</h1>
            <p className="text-sm text-zinc-500">{gin.requestedBy ?? "N/A"} | {format(new Date(gin.ginDate), "dd/MM/yyyy")}</p>
          </div>
        </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-4 gap-4">
                ...
                <div className="space-y-1"><Label>Date *</Label><Input type="date" {...form.register("ginDate")} /></div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-4 gap-6">
                <div><div className="text-xs text-zinc-500 mb-1">Requested By</div><div className="font-medium">{gin.requestedBy ?? "-"}</div></div>
                <div><div className="text-xs text-zinc-500 mb-1">Project</div><div className="font-medium">{gin.project?.projectNo ?? gin.projectNo ?? "-"}</div></div>
                <div><div className="text-xs text-zinc-500 mb-1">Date</div><div className="font-medium">{format(new Date(gin.ginDate), "dd/MM/yyyy")}</div></div>
                <div><div className="text-xs text-zinc-500 mb-1">Description</div><div className="font-medium">{gin.description ?? "-"}</div></div>
              </div>
            </CardContent>
          </Card>
      ...
      <ConfirmDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen} title="Delete GIN" description={`Delete GIN "${gin.ginNo}"? This action cannot be undone.`} onConfirm={() => deleteMutation.mutate()} isLoading={deleteMutation.isPending} />
    </div>
  );
}
