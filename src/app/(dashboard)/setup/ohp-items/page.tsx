...
type OhpItemFormData = z.infer<typeof ohpItemSchema>;

// --- Component ---

export default function OhpItemsPage() {
  ...
  const { data: itemsData, isLoading } = useQuery({
    queryKey: ["ohp-items"],
    queryFn: () => fetchApi<PaginatedResponse<Item>>("/api/setup/items?pageSize=200&itemType=OHP"),
  });
  ...
  const form = useForm<OhpItemFormData>({
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    resolver: zodResolver(ohpItemSchema) as any,
    defaultValues: {
      barcode: "",
      name: "",
      modelNo: "",
  ...
  const createMutation = useMutation({
    mutationFn: (data: OhpItemFormData) =>
      fetchApi<Item>("/api/setup/items", {
        method: "POST",
        body: JSON.stringify({ ...data, itemType: "OHP" }),
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ohp-items"] });
      toast.success("OHP item created successfully");
      handleCloseDialog();
    },
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ohp-items"] });
      toast.success("OHP item updated successfully");
      handleCloseDialog();
    },
    onError: (error: Error) => toast.error(error.message),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ohp-items"] });
      toast.success("OHP item deleted successfully");
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-semibold">OHP Items</h1>
        <p className="text-sm text-zinc-500">{itemsData?.total ?? 0} overhead project items total</p>
      </div>
      ...
      <DataTable
        ...
        data={itemsData?.data ?? []}
        searchKey="name"
        searchPlaceholder="Search OHP items..."
        isLoading={isLoading}
        emptyMessage="No OHP items found."
      ...
      <FormDialog
        ...
        title={editingItem ? "Edit OHP Item" : "Create OHP Item"}
